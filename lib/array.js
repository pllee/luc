...
function first(arr, howMany) {
    var end = howMany || 1,
        items =  arraySlice.call(arr,  0, end);

    return howMany === undefined ? items[0] : items;
}

function last(arr, h) {
    var howMany = h || 1,
        items =  arraySlice.call(arr,  howMany * -1);

    return h === undefined ? items[0] : items;
}
